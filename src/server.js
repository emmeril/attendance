...
const PERIOD_MODES = new Set(['previous_month', 'cutoff', 'manual']);
const HOLIDAY_ADJUSTMENTS = new Set(['none', 'previous_workday', 'next_workday']);
const DEFAULT_PAYROLL_SETTING = { payday_day: 10, period_mode: 'previous_month', cutoff_day: 25, holiday_adjustment: 'previous_workday', bpjs_health_employee_rate: 1, bpjs_health_wage_cap: 12000000, jht_employee_rate: 2, jp_employee_rate: 1, jp_wage_cap: 10547400 };
...
function addDays(value, amount) { const date = dateOnly(value); date.setUTCDate(date.getUTCDate() + amount); return dateString(date); }
function monthDays(year, month) { return new Date(Date.UTC(year, month, 0)).getUTCDate(); }
function shiftMonth(year, month, amount) { const date = new Date(Date.UTC(year, month - 1 + amount, 1)); return { year: date.getUTCFullYear(), month: date.getUTCMonth() + 1 }; }
function getPayrollSettings(year) { let row = db.prepare('SELECT * FROM payroll_settings WHERE year=?').get(year); if (!row) { db.prepare('INSERT OR IGNORE INTO payroll_settings(year) VALUES(?)').run(year); row = db.prepare('SELECT * FROM payroll_settings WHERE year=?').get(year); } return { ...DEFAULT_PAYROLL_SETTING, ...row }; }
function holidayMap(startDate, endDate) { return new Map(db.prepare('SELECT holiday_date,is_working_day FROM holidays WHERE holiday_date BETWEEN ? AND ?').all(startDate, endDate).map(row => [row.holiday_date, Number(row.is_working_day) === 1])); }
function isWeekend(value) { const day = dateOnly(value).getUTCDay(); return day === 0 || day === 6; }
function adjustedPaymentDate(paymentDate, adjustment, holidays) {
  if (adjustment === 'none') return paymentDate;
  let candidate = paymentDate;
  while (isWeekend(candidate) || (holidays.has(candidate) && !holidays.get(candidate))) candidate = addDays(candidate, adjustment === 'next_workday' ? 1 : -1);
  return candidate;
}
function payrollPreview(paymentDate, setting) {
  const payment = dateOnly(paymentDate); if (!payment) return null;
  const year = payment.getUTCFullYear(); const month = payment.getUTCMonth() + 1;
  let startDate = null; let endDate = null;
  if (setting.period_mode === 'previous_month') {
    const previous = shiftMonth(year, month, -1); startDate = `${previous.year}-${String(previous.month).padStart(2, '0')}-01`; endDate = `${previous.year}-${String(previous.month).padStart(2, '0')}-${String(monthDays(previous.year, previous.month)).padStart(2, '0')}`;
  } else if (setting.period_mode === 'cutoff') {
    const endMonth = shiftMonth(year, month, -1); const startMonth = shiftMonth(year, month, -2); const cutoff = Number(setting.cutoff_day) || 25; const endDay = Math.min(cutoff, monthDays(endMonth.year, endMonth.month)); const previousCutoff = `${startMonth.year}-${String(startMonth.month).padStart(2, '0')}-${String(Math.min(cutoff, monthDays(startMonth.year, startMonth.month))).padStart(2, '0')}`; startDate = addDays(previousCutoff, 1); endDate = `${endMonth.year}-${String(endMonth.month).padStart(2, '0')}-${String(endDay).padStart(2, '0')}`;
  }
  const holidays = holidayMap(addDays(paymentDate, -14), addDays(paymentDate, 14));
  return { payment_date: paymentDate, adjusted_payment_date: adjustedPaymentDate(paymentDate, setting.holiday_adjustment, holidays), start_date: startDate, end_date: endDate, period_mode: setting.period_mode, payday_day: setting.payday_day };
}

app.get('/api/holidays', isAuthenticated, (req, res) => { const year = Number(req.query.year || new Date().getFullYear()); res.json(db.prepare("SELECT * FROM holidays WHERE substr(holiday_date,1,4)=? ORDER BY holiday_date").all(String(year))); });
app.post('/api/holidays', isAuthenticated, (req, res) => { const date = String(req.body.holiday_date || ''); const name = String(req.body.name || '').trim(); if (!dateOnly(date) || !name) return res.status(400).json({ error: 'Tanggal dan nama hari libur wajib diisi' }); try { const result = db.prepare('INSERT INTO holidays (holiday_date,name,is_working_day) VALUES (?,?,?)').run(date, name, req.body.is_working_day ? 1 : 0); res.status(201).json({ id: result.lastInsertRowid }); } catch (error) { res.status(400).json({ error: error.message }); } });
app.put('/api/holidays/:id', isAuthenticated, (req, res) => { const date = String(req.body.holiday_date || ''); const name = String(req.body.name || '').trim(); if (!dateOnly(date) || !name) return res.status(400).json({ error: 'Tanggal dan nama hari libur wajib diisi' }); try { const result = db.prepare('UPDATE holidays SET holiday_date=?,name=?,is_working_day=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(date, name, req.body.is_working_day ? 1 : 0, req.params.id); if (!result.changes) return res.status(404).json({ error: 'Hari libur tidak ditemukan' }); res.json({ ok: true }); } catch (error) { res.status(400).json({ error: error.message }); } });
app.delete('/api/holidays/:id', isAuthenticated, (req, res) => { try { const result = db.prepare('DELETE FROM holidays WHERE id=?').run(req.params.id); if (!result.changes) return res.status(404).json({ error: 'Hari libur tidak ditemukan' }); res.json({ ok: true }); } catch (error) { res.status(400).json({ error: error.message }); } });
app.get('/api/payroll/settings', isAuthenticated, (req, res) => res.json(getPayrollSettings(Number(req.query.year || new Date().getFullYear()))));
app.put('/api/payroll/settings/:year', isAuthenticated, (req, res) => { const year=Number(req.params.year); const paydayValue=Number(req.body.payday_day); const cutoffValue=Number(req.body.cutoff_day); const paydayDay=Number.isInteger(paydayValue)&&paydayValue>=1&&paydayValue<=31?paydayValue:10; const cutoffDay=Number.isInteger(cutoffValue)&&cutoffValue>=1&&cutoffValue<=31?cutoffValue:25; const mode=String(req.body.period_mode||'previous_month'); const adjustment=String(req.body.holiday_adjustment||'previous_workday'); if(!PERIOD_MODES.has(mode)||!HOLIDAY_ADJUSTMENTS.has(adjustment)) return res.status(400).json({error:'Pengaturan tanggal payroll tidak valid'}); try { db.prepare(`INSERT INTO payroll_settings (year,payday_day,period_mode,cutoff_day,holiday_adjustment,bpjs_health_employee_rate,bpjs_health_wage_cap,jht_employee_rate,jp_employee_rate,jp_wage_cap,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(year) DO UPDATE SET payday_day=excluded.payday_day,period_mode=excluded.period_mode,cutoff_day=excluded.cutoff_day,holiday_adjustment=excluded.holiday_adjustment,bpjs_health_employee_rate=excluded.bpjs_health_employee_rate,bpjs_health_wage_cap=excluded.bpjs_health_wage_cap,jht_employee_rate=excluded.jht_employee_rate,jp_employee_rate=excluded.jp_employee_rate,jp_wage_cap=excluded.jp_wage_cap,updated_at=CURRENT_TIMESTAMP`).run(year,paydayDay,mode,cutoffDay,adjustment,Number(req.body.bpjs_health_employee_rate||1),Number(req.body.bpjs_health_wage_cap||12000000),Number(req.body.jht_employee_rate||2),Number(req.body.jp_employee_rate||1),Number(req.body.jp_wage_cap||10547400)); res.json({ok:true}); } catch(e){res.status(400).json({error:e.message});} });
app.get('/api/payroll/preview', isAuthenticated, (req, res) => { const paymentDate=String(req.query.payment_date||''); const setting=getPayrollSettings(Number(paymentDate.slice(0,4))||new Date().getFullYear()); const requestedMode=String(req.query.period_mode||setting.period_mode); if(PERIOD_MODES.has(requestedMode)) setting.period_mode=requestedMode; const preview=payrollPreview(paymentDate,setting); if(!preview) return res.status(400).json({error:'Tanggal pembayaran tidak valid'}); res.json(preview); });
...
app.post('/api/payroll/periods', isAuthenticated, (req, res) => { const x=req.body; try { const paymentDate=x.payment_date ? String(x.payment_date) : null; const setting=getPayrollSettings(Number((paymentDate||'').slice(0,4))||new Date().getFullYear()); const preview=paymentDate ? payrollPreview(paymentDate,{...setting,period_mode:PERIOD_MODES.has(String(x.period_mode))?String(x.period_mode):setting.period_mode}) : null; const startDate=String(x.start_date||preview?.start_date||''); const endDate=String(x.end_date||preview?.end_date||''); if(!x.name||!dateOnly(startDate)||!dateOnly(endDate)||endDate<startDate) return res.status(400).json({error:'Nama dan rentang periode payroll wajib benar'}); const result=db.prepare('INSERT INTO payroll_periods (name,start_date,end_date,payment_date,adjusted_payment_date,period_mode,created_by) VALUES (?,?,?,?,?,?,?)').run(x.name,startDate,endDate,paymentDate&&dateOnly(paymentDate)?paymentDate:null,preview?.adjusted_payment_date||null,preview?.period_mode||'manual',req.session.user.id); res.status(201).json({id:result.lastInsertRowid}); } catch(e){res.status(400).json({error:e.message});} });
app.put('/api/payroll/periods/:id/status', isAuthenticated, (req, res) => { const status=String(req.body.status||''); if(!['draft','calculated','locked','paid'].includes(status)) return res.status(400).json({error:'Status payroll tidak valid'}); const period=db.prepare('SELECT status FROM payroll_periods WHERE id=?').get(req.params.id); if(!period) return res.status(404).json({error:'Periode payroll tidak ditemukan'}); const transitions={draft:['calculated'],calculated:['locked'],locked:['paid'],paid:[]}; if(period.status!==status&&!transitions[period.status]?.includes(status)) return res.status(409).json({error:`Status payroll tidak dapat diubah dari ${period.status} ke ${status}`}); try { db.prepare('UPDATE payroll_periods SET status=?,paid_at=CASE WHEN ?=\'paid\' THEN CURRENT_TIMESTAMP ELSE paid_at END WHERE id=?').run(status,status,req.params.id); res.json({ok:true}); } catch(e){res.status(400).json({error:e.message});} });
app.post('/api/payroll/periods/:id/calculate', isAuthenticated, (req, res) => {
  const period=db.prepare('SELECT * FROM payroll_periods WHERE id=?').get(req.params.id); if(!period) return res.status(404).json({error:'Periode payroll tidak ditemukan'}); if(['locked','paid'].includes(period.status)) return res.status(409).json({error:'Payroll sudah dikunci dan tidak dapat dihitung ulang'});
  const year=Number(period.end_date.slice(0,4)); const setting=getPayrollSettings(year); const holidays=holidayMap(period.start_date,period.end_date); const employees=db.prepare('SELECT e.*,s.work_days FROM employees e LEFT JOIN shifts s ON s.id=e.shift_id WHERE e.is_active=1 ORDER BY e.name').all(); const upsert=db.prepare(`INSERT INTO payroll_records (payroll_period_id,employee_id,base_salary,gross_salary,unpaid_leave_deduction,attendance_days,scheduled_days,absence_days,late_minutes_total,bpjs_health_employee,jht_employee,jp_employee,pph21,net_salary) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(payroll_period_id,employee_id) DO UPDATE SET base_salary=excluded.base_salary,gross_salary=excluded.gross_salary,unpaid_leave_deduction=excluded.unpaid_leave_deduction,attendance_days=excluded.attendance_days,scheduled_days=excluded.scheduled_days,absence_days=excluded.absence_days,late_minutes_total=excluded.late_minutes_total,bpjs_health_employee=excluded.bpjs_health_employee,jht_employee=excluded.jht_employee,jp_employee=excluded.jp_employee,pph21=excluded.pph21,net_salary=excluded.net_salary`);
  const days=[]; for(let cursor=dateOnly(period.start_date),end=dateOnly(period.end_date);cursor<=end;cursor.setUTCDate(cursor.getUTCDate()+1)){days.push(dateString(cursor));}
  const calculate=db.transaction(()=>{for(const employee of employees){const workDays=String(employee.work_days||'1,2,3,4,5').split(',').map(Number);const scheduledDays=days.filter(day=>workDays.includes(dateOnly(day).getUTCDay())&&(!holidays.has(day)||holidays.get(day))).length;const attendance=db.prepare(`SELECT COUNT(*) days,COALESCE(SUM(late_minutes),0) late FROM daily_attendance WHERE employee_id=? AND attendance_date BETWEEN ? AND ? AND status IN ('hadir','terlambat')`).get(employee.id,period.start_date,period.end_date);const paidLeave=db.prepare(`SELECT COALESCE(SUM(r.total_days),0) days FROM leave_requests r JOIN leave_types t ON t.id=r.leave_type_id WHERE r.employee_id=? AND r.status='approved' AND t.is_paid=1 AND r.end_date>=? AND r.start_date<=?`).get(employee.id,period.start_date,period.end_date).days;const unpaidLeave=db.prepare(`SELECT COALESCE(SUM(r.total_days),0) days FROM leave_requests r JOIN leave_types t ON t.id=r.leave_type_id WHERE r.employee_id=? AND r.status='approved' AND t.is_paid=0 AND r.end_date>=? AND r.start_date<=?`).get(employee.id,period.start_date,period.end_date).days;const attendanceDays=Number(attendance.days||0);const absenceDays=Math.max(0,scheduledDays-attendanceDays-Number(paidLeave||0)-Number(unpaidLeave||0));const dailyRate=base=>Math.round(base/Math.max(1,scheduledDays));const base=Number(employee.base_salary||0);const unpaidDeduction=dailyRate(base)*(Number(unpaidLeave||0)+absenceDays);const health=Math.round(Math.min(base,setting.bpjs_health_wage_cap)*Number(setting.bpjs_health_employee_rate||0)/100);const jht=Math.round(base*Number(setting.jht_employee_rate||0)/100);const jp=Math.round(Math.min(base,setting.jp_wage_cap)*Number(setting.jp_employee_rate||0)/100);const gross=base;const pph=Math.round(gross*Number(employee.pph21_rate||0)/100);const net=Math.max(0,gross-unpaidDeduction-health-jht-jp-pph);upsert.run(period.id,employee.id,base,gross,unpaidDeduction,attendanceDays,scheduledDays,absenceDays,Number(attendance.late||0),health,jht,jp,pph,net);}}); calculate(); db.prepare("UPDATE payroll_periods SET status='calculated' WHERE id=?").run(period.id); res.json({ok:true,employees:employees.length});
});
