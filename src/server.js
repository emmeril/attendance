...
app.get('/api/attendance', isAuthenticated, (req, res) => { const to=String(req.query.to||new Date().toISOString().slice(0,10)); const fallbackFrom=new Date(`${to}T00:00:00Z`); fallbackFrom.setUTCDate(fallbackFrom.getUTCDate()-29); const from=String(req.query.from||fallbackFrom.toISOString().slice(0,10)); const rows=db.prepare(`SELECT da.*,e.employee_code,e.name,e.department,s.name shift_name FROM daily_attendance da JOIN employees e ON e.id=da.employee_id LEFT JOIN shifts s ON s.id=e.shift_id WHERE da.attendance_date BETWEEN ? AND ? ORDER BY da.attendance_date DESC,e.name`).all(from,to); res.json(rows); });
