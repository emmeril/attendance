...
app.post('/api/shifts', isAuthenticated, (req, res) => { const x=req.body; try { if(!x.name||!x.start_time||!x.end_time) return res.status(400).json({error:'Nama, jam masuk, dan jam pulang wajib diisi'}); const r=db.prepare('INSERT INTO shifts (name,start_time,end_time,late_tolerance_minutes,work_days) VALUES (?,?,?,?,?)').run(x.name,x.start_time,x.end_time,Number(x.late_tolerance_minutes||0),x.work_days||'1,2,3,4,5'); res.status(201).json({id:r.lastInsertRowid}); } catch(e) { res.status(400).json({error:e.message}); } });
app.put('/api/shifts/:id', isAuthenticated, (req, res) => { const x=req.body; try { if(!x.name||!x.start_time||!x.end_time) return res.status(400).json({error:'Nama, jam masuk, dan jam pulang wajib diisi'}); const result=db.prepare('UPDATE shifts SET name=?,start_time=?,end_time=?,late_tolerance_minutes=?,work_days=? WHERE id=?').run(x.name,x.start_time,x.end_time,Number(x.late_tolerance_minutes||0),x.work_days||'1,2,3,4,5',req.params.id); if(!result.changes) return res.status(404).json({error:'Jam kerja tidak ditemukan'}); res.json({ok:true}); } catch(e) { res.status(400).json({error:e.message}); } });
