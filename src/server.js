...
app.post('/api/employees', isAuthenticated, (req, res) => {
  const schema = z.object({ nik: z.string().min(1), name: z.string().min(1), department: z.string().optional(), position: z.string().optional(), email: z.string().optional(), phone: z.string().optional(), shift_id: z.preprocess((value) => value === '' || value == null ? undefined : Number(value), z.number().int().positive().optional()), device_user_id: z.string().optional() });
  const parsed = schema.safeParse(req.body); if (!parsed.success) return res.status(400).json({ error: parsed.error.issues[0].message });
  try { const x = parsed.data; const result = db.prepare(`INSERT INTO employees (employee_code,nik,name,department,position,email,phone,shift_id,device_user_id) VALUES (?,?,?,?,?,?,?,?,?)`).run(`AUTO-${Date.now()}`,x.nik,x.name,x.department||null,x.position||null,x.email||null,x.phone||null,x.shift_id||null,x.device_user_id||null); db.prepare('UPDATE employees SET employee_code=? WHERE id=?').run(`EMP-${String(result.lastInsertRowid).padStart(6,'0')}`,result.lastInsertRowid); saveEmployeeMachineMappings(result.lastInsertRowid, req.body.machine_mappings); res.status(201).json({ id: result.lastInsertRowid }); } catch (e) { res.status(400).json({ error: e.message }); }
});
app.put('/api/employees/:id', isAuthenticated, (req, res) => {
  const x = req.body; try { db.prepare(`UPDATE employees SET nik=?,name=?,department=?,position=?,email=?,phone=?,shift_id=?,device_user_id=?,is_active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(x.nik,x.name,x.department||null,x.position||null,x.email||null,x.phone||null,x.shift_id||null,x.device_user_id||null,x.is_active === false ? 0 : 1, req.params.id); saveEmployeeMachineMappings(req.params.id, x.machine_mappings); res.json({ ok: true }); } catch (e) { res.status(400).json({ error: e.message }); }
});
...
app.get('/api/attendance', isAuthenticated, (req, res) => { const to=String(req.query.to||new Date().toISOString().slice(0,10)); const fallbackFrom=new Date(`${to}T00:00:00Z`); fallbackFrom.setUTCDate(fallbackFrom.getUTCDate()-29); const from=String(req.query.from||fallbackFrom.toISOString().slice(0,10)); const rows=db.prepare(`SELECT da.*,e.nik,e.employee_code,e.name,e.department,s.name shift_name FROM daily_attendance da JOIN employees e ON e.id=da.employee_id LEFT JOIN shifts s ON s.id=e.shift_id WHERE da.attendance_date BETWEEN ? AND ? ORDER BY da.attendance_date DESC,e.name`).all(from,to); res.json(rows); });
