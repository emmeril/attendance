function attendanceApp() {
  ...
  return {
    ...
    async saveShift() { try { const editing=Boolean(this.editingShift); await this.request(editing?`/api/shifts/${this.editingShift.id}`:'/api/shifts',{method:editing?'PUT':'POST',body:JSON.stringify(this.shiftForm)}); this.shiftModal=false; this.editingShift=null; this.shiftForm={name:'',start_time:'08:00',end_time:'17:00',late_tolerance_minutes:10,work_days:'1,2,3,4,5'}; await this.loadShifts(); this.notify(editing?'Shift diperbarui.':'Shift berhasil disimpan.'); } catch(e) { this.notify(e.message); } },
    editShift(row) { this.editingShift=row; this.shiftForm={name:row.name||'',start_time:row.start_time||'08:00',end_time:row.end_time||'17:00',late_tolerance_minutes:row.late_tolerance_minutes||0,work_days:row.work_days||'1,2,3,4,5'}; this.shiftModal=true; },
