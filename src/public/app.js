function attendanceApp() {
  ...
  return {
    ...
    shiftForm: { name:'', start_time:'08:00', end_time:'17:00', late_tolerance_minutes:10, work_days:'1,2,3,4,5' }, shiftDays: ['1','2','3','4','5'],
    ...
    async saveEmployee() { try { const editing=Boolean(this.editingEmployee); await this.request(editing?`/api/employees/${this.editingEmployee.id}`:'/api/employees',{method:editing?'PUT':'POST',body:JSON.stringify(this.employeeForm)}); this.employeeModal=false; this.editingEmployee=null; this.employeeForm={employee_code:'',name:'',department:'',position:'',shift_id:'',device_user_id:''}; await Promise.all([this.loadEmployees(),this.loadDashboard()]); this.notify(editing?'Karyawan diperbarui.':'Karyawan berhasil disimpan.'); } catch(e) { this.notify(e.message); } },
    editEmployee(row) { this.editingEmployee=row; this.employeeForm={employee_code:row.employee_code||'',name:row.name||'',department:row.department||'',position:row.position||'',shift_id:row.shift_id||'',device_user_id:row.device_user_id||''}; this.employeeModal=true; },
    ...
    async saveShift() { try { const editing=Boolean(this.editingShift); if(!this.shiftForm.start_time || !this.shiftForm.end_time) throw new Error('Jam masuk dan jam pulang wajib diisi.'); if(this.shiftForm.end_time < this.shiftForm.start_time) throw new Error('Jam pulang tidak boleh lebih awal dari jam masuk.'); if(!this.shiftDays.length) throw new Error('Pilih minimal satu hari kerja.'); const payload={...this.shiftForm,work_days:this.shiftDays.join(',')}; await this.request(editing?`/api/shifts/${this.editingShift.id}`:'/api/shifts',{method:editing?'PUT':'POST',body:JSON.stringify(payload)}); this.shiftModal=false; this.editingShift=null; this.shiftForm={name:'',start_time:'08:00',end_time:'17:00',late_tolerance_minutes:10,work_days:'1,2,3,4,5'}; this.shiftDays=['1','2','3','4','5']; await this.loadShifts(); this.notify(editing?'Jam kerja diperbarui.':'Jam kerja berhasil disimpan.'); } catch(e) { this.notify(e.message); } },
    editShift(row) { this.editingShift=row; this.shiftForm={name:row.name||'',start_time:row.start_time||'08:00',end_time:row.end_time||'17:00',late_tolerance_minutes:row.late_tolerance_minutes||0,work_days:row.work_days||'1,2,3,4,5'}; this.shiftDays=String(row.work_days||'').split(',').filter(Boolean); this.shiftModal=true; },
    async deleteShift(row) { if(!confirm(`Hapus shift ${row.name}?`)) return; try { await this.request(`/api/shifts/${row.id}`,{method:'DELETE'}); await Promise.all([this.loadShifts(),this.loadEmployees()]); this.notify('Shift dihapus.'); } catch(e) { this.notify(e.message); } },
    ...
    newAttendance() { this.editingAttendance=null; this.attendanceForm={id:null,employee_id:'',attendance_date:this.date,check_in:'',check_out:'',status:'hadir',late_minutes:0,work_minutes:0,notes:''}; this.attendanceModal=true; },
