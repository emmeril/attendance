function attendanceApp() {
  ...
  return {
    ...
    async syncDevice(device) { try { const result=await this.request(`/api/devices/${device.id}/sync`, { method:'POST', body:JSON.stringify({from:this.from,to:this.to}) }); const userText=result.users ? ` ${result.users.inserted} karyawan baru, ${result.users.updated} diperbarui.` : ''; this.notify(`${result.inserted} data baru disinkronkan.${userText}`); await Promise.all([this.loadDashboard(),this.loadAttendance(),this.loadEmployees()]); } catch(e) { this.notify(`Sinkronisasi gagal: ${e.message}`); } },
